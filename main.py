...
def admin_menu_kb() -> InlineKeyboardMarkup:
    ...
    b.row(InlineKeyboardButton(text="📢 Рассылка",            callback_data="admin_broadcast_start"))
    return b.as_markup()
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    text = (
        "👋 <b>Привет!</b>\n\n"
        "Я бот-модератор подписок. Я помогаю проверять подписку участников "
        "на каналы-спонсоры прямо в ваших чатах.\n\n"
        "<b>Что я умею:</b>\n"
        "• Проверять подписку при каждом сообщении\n"
        "• Удалять сообщения тех, кто не подписан\n"
        "• Показывать кнопки для быстрой подписки\n"
        "• Работать в режимах FREE и PRO\n\n"
        "<b>Как добавить чат:</b>\n"
        "• Напишите <code>/add</code> прямо в своём чате — это быстрее всего\n"
        "• Или узнайте ID чата командой <code>/id</code> и введите его здесь\n\n"
        "Начните с добавления вашего чата 👇"
    )
...
async def help_handler(callback: CallbackQuery):
    text = (
        ...
        "<b>1. Добавление чата (3 способа):</b>\n"
        "   🥇 <b>Способ 1 — /add в чате (проще всего):</b>\n"
        "   • Добавьте бота в чат и назначьте его администратором\n"
        "   • Напишите в чате <code>/add</code> — чат зарегистрируется автоматически\n\n"
        "   🥈 <b>Способ 2 — по Chat ID:</b>\n"
        "   • Напишите в чате команду <code>/id</code>\n"
        "   • Скопируйте ID и введите его в боте через ➕ Добавить чат\n\n"
        "<b>2. Узнать ID чата:</b>\n"
        "   • Напишите <code>/id</code> в любом чате с ботом\n"
        "   • ID придёт в чат и вам в личку\n\n"
        "<b>3. Настройка спонсоров:</b>\n"
        "   • Перешлите сообщение из канала-спонсора боту\n"
        "   • Укажите ссылку на канал для кнопки\n\n"
        "<b>4. Режимы работы:</b>\n"
        "   • <b>FREE</b> — бот проверяет подписку и удаляет сообщения нарушителей\n"
        "   • <b>PRO</b> — глобальный спонсор отключён, проверяются только ваши спонсоры\n\n"
        "<b>5. Защита от отписки:</b>\n"
        "   Проверка происходит при каждом сообщении. Если пользователь "
        "отпишется — его следующее сообщение будет удалено."
    )
    b = InlineKeyboardBuilder()
...
async def add_chat_start(callback: CallbackQuery, state: FSMContext):
    b = InlineKeyboardBuilder()
    b.row(InlineKeyboardButton(text="🆔 Добавить по Chat ID", callback_data="add_chat_by_id"))
    b.row(back_btn("main_menu"))
    await callback.message.edit_text(
        "📨 <b>Добавление чата</b>\n\n"
        "<b>Способ 1 — команда /add прямо в чате (проще всего):</b>\n"
        "1. Добавьте бота в ваш чат\n"
        "2. Назначьте бота администратором (с правом удаления сообщений)\n"
        "3. Напишите в чате команду <code>/add</code>\n"
        "4. Бот автоматически зарегистрирует чат на ваш аккаунт\n\n"
        "<b>Способ 2 — по Chat ID:</b>\n"
        "1. Напишите в чате <code>/id</code> — бот пришлёт ID чата\n"
        "2. Нажмите кнопку ниже и введите ID\n\n"
        "💡 <b>Узнать ID чата:</b> напишите <code>/id</code> в любом чате где есть бот",
...
async def _do_add_sponsor(chat_id: str, channel_id: int, title: str, link: str | None, owner_id: int,
                          sponsor_type: str = "channel", bot_token: str | None = None) -> str:
...
async def add_sponsor_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "📨 <b>Переслать сообщение</b> — только для каналов\n"
        "🆔 <b>Ввести ID вручную</b> — для каналов и групп\n"
        "🤖 <b>Добавить бота</b> — проверяет что пользователь запустил бота",
        reply_markup=b.as_markup()
    )
...
async def process_sponsor_forward(message: Message, state: FSMContext):
    ...
    default_link = f"https://t.me/{username}" if username else None

    await state.update_data(
        sponsor_channel_id=channel.id,
        sponsor_channel_title=channel.title,
        sponsor_default_link=default_link,
    )
    ...
    if default_link:
        await message.answer(
            ...
            f"Ссылка для кнопки: <code>{default_link}</code>\n\n"
            "Отправьте другую ссылку, или /skip чтобы использовать эту."
        )
    else:
        await message.answer(
            f"✅ Канал найден: <b>{channel.title}</b> (приватный)\n\n"
            "Введите пригласительную ссылку для кнопки, или /skip чтобы добавить без ссылки."
        )
...
async def process_sponsor_link(message: Message, state: FSMContext):
    ...
    default_lnk = data.get("sponsor_default_link")

    if message.text and message.text.strip() == "/skip":
        link = default_lnk
    else:
        link = message.text.strip() if message.text else default_lnk
...
async def process_sponsor_id_input(message: Message, state: FSMContext):
    ...
    if default_link:
        ...
        await message.answer(
            f"✅ Найдено: <b>{chat_info.title}</b> (приватный)\n\n"
            "Введите пригласительную ссылку для кнопки, или /skip чтобы добавить без ссылки."
        )
